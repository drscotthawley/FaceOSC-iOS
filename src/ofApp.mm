...
void ofApp::loadSettings() {
    ...
    if(xml.exists("device")) {
        cam.setDeviceID(xml.getValue("device", 1)); // 1 for front-side camera
        ready = true;
    }
    if(xml.exists("framerate")) {
        cam.setDesiredFrameRate(xml.getValue("framerate", 30));
    }
    camWidth = xml.getValue("width", 480);
    camHeight = xml.getValue("height", 640);
...
void ofApp::draw() {
    ...
    // Next two lines reverse the appearance of the face (for front-facing camera)
    ofTranslate(ofGetWidth(), 0);    // move to right edge of screen
    ofScale(-1, 1);                  // flip everything horizontally
    
    // these lines make the camera image bigger
    float scale = ofGetWidth() / cam.getWidth();
    ofScale(scale, scale);
    
    videoSource->draw(0, 0);        // draw what's coming from the camera
    
    // TODO: somehow need to 'reverse back' for text so it's not all backwards
    //..or just not show the gui
